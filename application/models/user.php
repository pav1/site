...
class user extends model {
	...
	function create( $email, $password, $first_name, $last_name){ 
        ...
        $sql = "INSERT INTO users (email, password,first_name,last_name,created_at) VALUES (?, SHA(?),?,?,NOW())";
            $statement_insert_admin = $database_link->prepare($sql);
            $statement_insert_admin->bind_param('ssss', $email, $password, $first_name,$last_name);
            ...
            $statement_add_user_to_role = $database_link->prepare($sql);
	...
	function update( $user_id, $email, $password, $first_name, $last_name){ 
		$sql = "UPDATE users SET email = '$email', password = SHA('$password'), first_name = '$first_name', last_name = '$last_name' WHERE id = '$user_id' LIMIT 1";
	...
	function find_by_email_and_password( $email, $password ){
		$sql = "SELECT * FROM users WHERE email = '$email' AND password = SHA('$password') LIMIT 1";
